...
(*目的：global_ekimei_listのローマ字から漢字の駅名を返す間数*)
(*romaji_to_kanji: romanji ekimei_t -> kanji*)
let rec romaji_to_kanji s_ekimei_roma lst = match lst with
  | [] -> ""
  | {kanji = s_kanji; kana = s_kana; romaji = s_roma; shozoku = s_shozoku} :: rest ->
    if s_roma = s_ekimei_roma then s_kanji else romaji_to_kanji s_ekimei_roma rest;;

(*test:romaji_to_kanji*)
let test1 = romaji_to_kanji "takyo" global_ekimei_list = "";;
let test2 = romaji_to_kanji "myogadani" global_ekimei_list = "茗荷谷";;

(*目的：駅名２つから駅間の距離を返す*)
(*get_ekikan_kyori : kanji kanji ekikan_t -> float*)
let rec get_ekikan_kyori ekimei1 ekimei2 lst = match lst with
  | [] -> infinity
  | {kiten = s_kiten; shuten = s_shuten; keiyu = s_keiyu; kyori = f_kyori; jikan = f_jikan} :: rest ->
    if ekimei1 = s_kiten && ekimei2 = s_shuten || ekimei2 = s_kiten && ekimei1 = s_shuten
    then f_kyori
    else get_ekikan_kyori ekimei1 ekimei2 rest;;

(*test: get_ekikan_kyori*)
